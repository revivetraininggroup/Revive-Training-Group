...
export default function ClientMessagesPage() {
    ...
    async function init() {
      ...
      if (clientRecord?.coach_id) {
        ...
        const { data: msgs } = await supabase
          ...
          .or(`and(sender_id.eq.${user!.id},recipient_id.eq.${clientRecord.coach_id}),and(sender_id.eq.${clientRecord.coach_id},recipient_id.eq.${user!.id})`)
          .order('sent_at')
        setMessages(msgs ?? [])

        // Mark as read
        await supabase.from('messages').update({ read: true }).eq('recipient_id', user!.id).eq('sender_id', clientRecord.coach_id)

        // Subscribe to new messages
        supabase.channel('client-messages').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, () => init()).subscribe()
      }
    }
    init()
  }, [])
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    setSending(true)
    await supabase.from('messages').insert({ sender_id: clientId, recipient_id: coach.id, content: newMsg })
    setNewMsg('')
    setSending(false)
  }
